...
def sim(plot_condition = False, condition_guidance = False, example = False, final_plot = False, time = None, flux = None, flux_error = None, temporalbin = None, PSDmodel = None, PSDparams = None, source_name_title = None, spec = None):
    ...
    if not condition_guidance and not example and time == None and flux == None and spec == None:
        print('You did not insert any parameters, in this case an example with PG 1553+113 will be shown to you')
        example = True
    ...
    if PSDmodel == None:
        ...
        if PSDparams == None:
            PSDparams = [1,1,-(p1*0.75),-(p1),0.0]
    if condition_guidance:
        print(PSDmodel+' PSD, parameters: ', PSDparams)
    ...
    while not converge:
        ...
        if j_index == 1 and condition_guidance:
            print(' ')
        ...
        if j_index == 1:
            ...
            if condition_guidance:
                ...
                print('when the two series converge it means we have obtained the Emmanoulopoulos simulated light curve x_Final, with the same PSD and PDF of the original source.')
                print(' ')
